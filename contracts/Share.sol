...
contract Share is ERC721Holder {
    ...
    function addShareholder(address payable _shareholder, uint256 _shares) external {
        ...
        if (addressToIndex[_shareholder] == 0) {
            ...
            shareholders[nextShareholderIndex] = Shareholder(_shareholder, _shares);
            nextShareholderIndex++;
        } else {
            uint256 index = addressToIndex[_shareholder];
            shareholders[index].shares = shareholders[index].shares.add(_shares);
        }
    ...
    function getShareholders() external view returns (ShareholderInfo[] memory) {
        ShareholderInfo[] memory shareholdersInfo = new ShareholderInfo[](nextShareholderIndex - 1);

        for (uint256 i = 1; i < nextShareholderIndex; i++) {
            Shareholder storage shareholder = shareholders[i];
            shareholdersInfo[i - 1] = ShareholderInfo(shareholder.shareholder, shareholder.shares);
        }
